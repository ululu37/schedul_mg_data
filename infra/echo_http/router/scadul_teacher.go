...
func RegisterScadulTeacherRoutes(e *echo.Echo, uc *usecase.ScadulTeacherMg) {
	g := e.Group("/scadulteacher")

	g.POST("", func(c echo.Context) error {
		var req entities.ScadulTeacher
	...
	g.POST("/:id/subject", func(c echo.Context) error {
		...
		var req []entities.SubjectInScadulTeacher
		if err := c.Bind(&req); err != nil {
			return c.JSON(http.StatusBadRequest, err.Error())
		}
		for i := range req {
			req[i].ScadulTeacherID = uint(id)
		}
